...
; * [NOTE]  The content from parentProcess above loadTop should run only once.
	...
	enteredlossCheck:
		...
		winCheck:
		cmp BYTE [collidedTile], 'W' ; * {534} As of backup 533, this code line has not yet been tested.
		jne noLossNorWin
...
;        dec     ecx ; * {352} This code line was disabled in backup 352.
	stop1:
        mov     ebx,python ; * {329} The label "message" in this line was swapped with the label "python" as of backup 328 or earlier. ; {332} In backup 332, this code was changed from backup 331's "-- mov     ebx,python+((pythonLen-pythonBody)-1)*pStruct.size --" to "-- mov     ebx,python --". ; * DEBUG DISABLE
	stop2:
...
;        mov     [ebx + pStruct.row],dx                  ;; copy to this row ; * {319} This line suggests row access is active. ; {350} This code line was disabled in backup 350.
	...
	mov	dx, [ebx + pStruct.row] ; * DEBUG DISABLE
	; Store the python's head's row data
	stop3:
	mov	[pythonHeadAsciiRow], dx ; * DEBUG DISABLE
	stop4:
...
;        mov     [ebx + pStruct.col],dx                  ;; copy to this col ; * {319} This line suggests column access is active. ; {351} This code line was disabled in backup 351.
	...
	mov	dx, [ebx + pStruct.col] ; * DEBUG DISABLE
	stop5:
	; Store the python head's column data
	mov	[pythonHeadAsciiColumn], dx ; * DEBUG DISABLE
	stop6:
...
;	mov al, [screen]; {486} Backup 486 attempt ; {488} Assembly attempt successful in backup 486, success recorded in program log in backup 487 ; * {493} Disabled in backup 493 to allow for additional code testing
	...
	mov bx, [pythonHeadIntRow] ; * {M8} Removal of brackets denotes assembler trucation ; {M10} Change from bl to bx
	mov al, 81
	mul bl

	add ax, [pythonHeadIntColumn] ; * {M8} Removal of brackets denotes assembler truncation
